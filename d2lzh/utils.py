...
class BiRNN(nn.Block):
    def __init__(self, vocad, embed_size, num_hiddens, num_layers, **kwargs):
        # 首先找到BiRNN的父类，把BiRNN的对象self转换为父类的对象
        # 子类把父类的__init__()放到自己的__init__()当中，这样子类就有了父类的__init__()的东西
        super(BiRNN, self).__init__(**kwargs)
        self.embedding = nn.Embedding(len(vocad), embed_size)
        # bidrectional设为true，就是双向循环神经网络
...
def read_imdb(folder='train'):
    ...
    for label in ['pos', 'neg']:
        nor_path = os.path.normpath(r'E:\program\python\SentimentAnalysis\data\aclImdb\aclImdb')
        folder_name = os.path.join(nor_path, folder, label)
...
class TextCNN(nn.Block):
    ...
    def forward(self, inputs):
        #将两个形状事（批量大小，词数，词向量维度）的嵌入层的输出按词向量连结
        embeddings = nd.concat(
            self.embedding(inputs), self.constant_embedding(inputs), dim = 2)
        #根据Conv1D的输入格式，将一维卷积层的通道维变换到前一维
        embeddings = embeddings.transpose((0, 2, 1))
        #对于每个一维卷积层，在时序最大池化后会得到一个性状为（批量大小， 通道大小， 1）的NDArray
        #使用flatten去掉最后一维，然后在通道维上连结
